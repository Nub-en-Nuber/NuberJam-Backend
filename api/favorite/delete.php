...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($response["status"] == $constant->RESPONSE_STATUS["success"]) {
        if (isset($_GET['musicId']) && isset($_GET['accountId'])) {
            $musicId = $_GET['musicId'];
            $accountId = $_GET['accountId'];

            $query = "DELETE FROM favorite WHERE musicId = '$musicId' AND accountId = '$accountId'";
            ...
            $response['message'] = $constant->RESPONSE_MESSAGES["music_and_account_id_needed"];
        }
    }
} else {
    $response["status"] = $constant->RESPONSE_STATUS["bad_request"];
